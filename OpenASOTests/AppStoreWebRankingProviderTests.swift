...
struct AppStoreWebRankingProviderTests {
    ...
    @Test
    func hydratesOrderedWebReferencesWithOneBoundedLookup() async throws {
        var requestedHosts: [String] = []
        let client = MockHTTPClient { request in
            let url = try #require(request.url)
            requestedHosts.append(try #require(url.host()))
            ...
            case "itunes.apple.com":
                ...
                #expect(request.timeoutInterval == 20)
                return (
                    Self.hydrationLookupFixture,
                    makeHTTPURLResponse(url: url, statusCode: 200)
                )
            default:
                Issue.record("Unexpected hydration host: \(url)")
                return (
                    Data(),
        ...
        #expect(requestedHosts == ["apps.apple.com", "itunes.apple.com"])
        #expect(page.source == .appStoreWeb)
        #expect(page.fallbackContext == nil)
        #expect(page.items.map(\.appStoreID) == [30, 10, 20])
        #expect(page.items.map(\.position) == [1, 2, 3])
        #expect(page.items.map(\.name) == ["Thirty", "Ten", "Twenty"])
        #expect(page.items.allSatisfy { $0.platform == .iphone })
    }

    @Test
    func hydratesContinuationReferencesAfterTheRenderedPrefix() async throws {
        let client = MockHTTPClient { request in
            let url = try #require(request.url)
            switch url.host() {
            case "apps.apple.com":
                return (
                    Self.continuationHydrationFixture,
                    makeHTTPURLResponse(url: url, statusCode: 200)
                )
            case "itunes.apple.com":
                let components = try #require(
                ...
                #expect(components.queryItems?.first == URLQueryItem(
                    name: "id",
                    value: "30,10,20"
                ))
                return (
    ...
    func rejectsDecoyShelvesAndChangedAuthoritativeShapes() async throws {
        ...
        let mismatchedReferencesClient = MockHTTPClient { request in
            let url = try #require(request.url)
            return (
                Self.mismatchedReferencesFixture,
                makeHTTPURLResponse(url: url, statusCode: 200)
            )
        }
        ...
        await Self.expectResponseFailure(.pageShapeChanged) {
            _ = try await AppStoreWebRankingProvider(httpClient: mismatchedReferencesClient).search(
                keyword: "notes",
                storefrontCode: "us",
                platform: .iphone,
                limit: 2
    ...
    func fallsBackOnlyForRecognizedPrimaryFailures() async throws {
        let recognizedFailures: [ScriptedRankingProvider.Behavior] = [
            ...
            .response(.lookupHydrationIncomplete),
        ]
    ...
    private static let mismatchedReferencesFixture = Data("""
    <script id="serialized-server-data">
    {"data":[{"intent":{"term":"notes","storefront":"us","platform":"iphone","$kind":"SearchResultsPageIntent"},"data":{"$kind":"SearchResultsPage","nextPage":{"results":[
      {"id":"10","type":"apps"},
      {"id":"30","type":"apps"},
      {"id":"20","type":"apps"}
    ]},"shelves":[{"id":"SearchResults.shelfId","$kind":"Shelf","contentType":"searchResult","items":[
      {"$kind":"AppSearchResult","resultType":"content","lockup":{"adamId":"30","title":"Unexpected First App"}}
    ]}]}}]}
    </script>
    """.utf8)

    private static let continuationHydrationFixture = Data("""
    <script id="serialized-server-data">
    {"data":[{"intent":{"term":"notes","storefront":"us","platform":"iphone","$kind":"SearchResultsPageIntent"},"data":{"$kind":"SearchResultsPage","nextPage":{"results":[
      {"id":"20","type":"apps","href":"/v1/catalog/us/apps/20"},
      {"id":"900","type":"editorial-items","href":"/v1/editorial/us/editorial-items/900"},
      {"id":"40","type":"apps","href":"/v1/catalog/us/apps/40"}
    ]},"shelves":[{"id":"SearchResults.shelfId","$kind":"Shelf","contentType":"searchResult","items":[
      {"$kind":"AppSearchResult","resultType":"content","lockup":{"adamId":"30","title":"Rendered Thirty"}},
      {"$kind":"EditorialSearchResult","resultType":"editorial"},
      {"$kind":"AppEventSearchResult","resultType":"appEvent","lockup":{"adamId":"10","title":"Rendered Ten"}}
    ]}]}}]}
    </script>
    """.utf8)

    private static let hydrationFixture = Data("""
    <script id="serialized-server-data">
    {"data":[{"intent":{"term":"notes","storefront":"us","platform":"iphone","$kind":"SearchResultsPageIntent"},"data":{"$kind":"SearchResultsPage","nextPage":{"results":[
      {"id":"30","type":"apps","href":"/v1/catalog/us/apps/30"},
      {"id":"900","type":"editorial-items","href":"/v1/editorial/us/editorial-items/900"},
      {"id":"10","type":"apps","href":"/v1/catalog/us/apps/10"},
      {"id":"901","type":"app-bundles","href":"/v1/catalog/us/app-bundles/901"},
      {"id":"902","type":"in-apps","href":"/v1/catalog/us/in-apps/902"},
      {"id":"20","type":"apps","href":"/v1/catalog/us/apps/20"},
      {"id":"40","type":"apps","href":"/v1/catalog/us/apps/40"}
    ]},"shelves":[{"id":"SearchResults.shelfId","$kind":"Shelf","contentType":"searchResult","items":[
      {"$kind":"AppSearchResult","resultType":"content","lockup":{"adamId":"30","title":"Rendered Thirty"}},
    ...
    private static let hydrationLookupFixture = Data("""
    {"results":[
      {"trackId":20,"trackName":"Twenty","bundleId":"com.example.twenty","sellerName":"Example"},
      {"trackId":30,"trackName":"Thirty","bundleId":"com.example.thirty","sellerName":"Example"},
      {"trackId":10,"trackName":"Ten","bundleId":"com.example.ten","sellerName":"Example"}
    ]}
    """.utf8)

    private static let incompleteHydrationLookupFixture = Data("""
    {"results":[
      {"trackId":30,"trackName":"Thirty"},
