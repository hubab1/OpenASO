...
struct OpenASOMCPServerTests {
    ...
    func serverExposesToolsResourcesPromptsAndReturnsJSONToolContent() async throws {
        ...
        #expect(prompts.map(\.name).contains("aso_action_plan"))
        #expect(prompts.map(\.name).contains("localization_opportunity_analysis"))
        ...
        #expect(String(describing: prompt.messages).contains("bounded query"))
