...
final class AppStoreWebRankingProvider: SearchRankingProvider {
    ...
    func search(
        ...
        return try Self.parse(data, request: validatedRequest)
    }
    ...
    private static func parse(
        ...
        if items.count < request.limit {
            // Apple server-renders only an initial shelf. A non-null nextPage means
            // returning this slice as a complete ranking would produce false misses.
            guard page.hasNextPageField else {
                throw SearchRankingProviderError.responseFailure(.pageShapeChanged)
            }
            guard page.nextPage == nil else {
